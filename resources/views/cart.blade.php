        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center gap-2 text-sm text-[#9CA3AF] mb-6">
                <a href="{{ route('lang.index', ['lang' => $langCode]) }}"
                    class="hover:text-[#8B5CF6] transition-colors">{{ __('buttons.home') }}</a>
                <i class="fas fa-chevron-right text-[10px]"></i>
                <a href="{{ route('checkout', ['lang' => $langCode]) }}"
                    class="hover:text-[#8B5CF6] transition-colors">{{ __('buttons.shop') }}</a>
                <i class="fas fa-chevron-right text-[10px]"></i>
            ...
            @if (quote() && quote()->quoteItems && quote()->quoteItems->count() > 0)
                        ...
                        <table class="w-full">
                            <thead>
                                <tr class="border-b border-[rgba(255,255,255,0.06)]">
                                    <th
                                        class="text-left py-4 px-6 text-xs font-medium text-[#9CA3AF] uppercase tracking-wider">
                                        {{ __('buttons.product') }}</th>
                                    <th
                                        class="text-left py-4 px-6 text-xs font-medium text-[#9CA3AF] uppercase tracking-wider">
                                        {{ __('buttons.price') }}</th>
                                    <th
                                        class="text-left py-4 px-6 text-xs font-medium text-[#9CA3AF] uppercase tracking-wider">
                                        {{ __('buttons.quantity') }}</th>
                                    <th
                                        class="text-left py-4 px-6 text-xs font-medium text-[#9CA3AF] uppercase tracking-wider">
                                        {{ __('buttons.total') }}</th>
                                    <th
                                        class="text-right py-4 px-6 text-xs font-medium text-[#9CA3AF] uppercase tracking-wider">
                                        {{ __('buttons.remove') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                    ...
                                    <tr
                                        ...
                                        <td class="py-4 px-6">
                                            <div class="flex items-center gap-4">
                                                <img src="{{ $item->product->getFirstMediaUrl('image') }}"
                                        ...
                                        <td class="py-4 px-6 text-sm text-[#F1F1F6]">
                                            {{ $symbol }}{{ number_format($item->price * $rate, 2) }}</td>
                                        <td class="py-4 px-6">
                                            <div
                                                class="flex items-center glass rounded-lg border border-[rgba(255,255,255,0.06)] w-fit">
                                                <form
                                                    ...
                                                    <button type="submit" name="qty"
                                                        ...
                                                        class="px-3 py-1.5 text-[#9CA3AF] hover:text-white transition-colors text-sm"><i
                                                            class="fas fa-minus"></i></button>
                                                    <span
                                                        class="px-3 py-1.5 text-white font-medium text-sm min-w-[30px] text-center">{{ $item->qty }}</span>
                                        ...
                                        <td class="py-4 px-6 text-right">
                                            <form action="{{ route('remove', $item->id) }}" method="POST">
                                                ...
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="w-8 h-8 flex items-center justify-center rounded-lg text-[#EF4444] hover:bg-[#EF4444]/10 transition-all">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
                    ...
                    <div class="glass rounded-2xl p-6 border border-[rgba(255,255,255,0.06)]">
                        ...
                        @php
                            $subtotal = quote()->quoteItems->sum(function ($item) use ($rate) {
                        ...
                        <div class="space-y-3 text-sm">
                            ...
                            <div class="border-t border-[rgba(255,255,255,0.06)] pt-3 flex justify-between">
                                ...
                                <span
                                    class="text-white font-bold text-lg">{{ $symbol }}{{ number_format($subtotal, 2) }}</span>
                            </div>
                        </div>
