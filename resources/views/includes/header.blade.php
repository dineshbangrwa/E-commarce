                ...
                <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                    <div class="custom-select-box">
                        <form id="currency-change-form" action="{{ route('currency.change') }}" method="POST">
                            ...
                            <select name="currency_code"
                                onchange="document.getElementById('currency-change-form').submit();"
                    ...
                    <div class="our-link">
                        <ul>
                            @if (Auth::check())
                                <li>
                                    <a
                                        href="{{ route('logout', ['lang' => session('language_code', app()->getLocale())]) }}">Logout</a>
                                </li>
                            @else
                                <li>
                                    <a
                                        href="{{ route('login', ['lang' => session('language_code', app()->getLocale())]) }}">Login</a>
                                </li>
                            @endif

                            <li><a href="#">Our location</a></li>
                            <li><a
                                    href="{{ route('contact', ['lang' => session('language_code', app()->getLocale())]) }}">Contact
                                    Us</a></li>
                        </ul>
                    ...
                    {{-- <div id="google_translate_element"></div>
                            ...
                            new google.translate.TranslateElement({
                                pageLanguage: 'en',
                                includedLanguages: 'en,hi,ja,zh-CN,ur', 
    ...
    <header class="main-header">
        <nav class="navbar navbar-expand-lg navbar-light bg-light navbar-default bootsnav">
            <div class="container">
                <div class="navbar-header">
                    ...
                    <a class="navbar-brand" href="{{ route('lang.index', ['lang' => $langCode]) }}">
                        <img src="{{ asset('front/images/logo.png') }}?v={{ time() }}" class="logo"
                            loading="lazy" alt="">
                    </a>
                ...
                <div class="collapse navbar-collapse" id="navbar-menu">
                    <ul class="nav navbar-nav ml-auto" data-in="fadeInDown" data-out="fadeOutUp">
                        <li class="nav-item {{ request()->routeIs('index') ? 'active' : '' }}">
                            <a class="nav-link"
                                href="{{ route('lang.index', ['lang' => $langCode]) }}">{{ __('buttons.home') }}</a>
                        </li>

                        @foreach (page() as $page)
                            <li class="nav-item {{ request()->is('page/' . $page->url_key) ? 'active' : '' }}">
                                <a class="nav-link"
                                    href="{{ route('page', ['lang' => $langCode, 'url_key' => $page->url_key]) }}">
                                    {{ $page->name }}
                                </a>
                            </li>
                        @endforeach
                        ...
                        <li class="dropdown megamenu-fw">
                            <a href="#" class="nav-link dropdown-toggle arrow"
                                data-toggle="dropdown">{{ __('buttons.categories') }}</a>
                                    ...
                                    <div class="row">
                                        @foreach (category1() as $category)
                                            @php
                                                ...
                                                $active = request()->is('category/' . $category->url_key);
                                                foreach ($subcategories as $subcat) {
                                                    if (request()->is('category/' . $subcat->url_key)) {
                                                        $active = true;
                                                        break;
                                                    }
                                                }
                                                    ...
                                                    @if ($subcategories->count() > 0)
                                                        <ul class="menu-col">
                                                            @foreach ($subcategories as $subcategory)
                                                                <li>
                                                                    <a
                                                                        href="{{ route('category', ['lang' => $langCode, 'url_key' => $subcategory->url_key]) }}">
                                                                        {{ $subcategory->name }}
                                                                    </a>
                                                                </li>
                                                            @endforeach
                        ...
                        <li class="nav-item {{ request()->routeIs('contact') ? 'active' : '' }}">
                            <a class="nav-link"
                                href="{{ route('contact', ['lang' => $langCode]) }}">{{ __('buttons.contact_us') }}</a>
                        </li>
                ...
                <div class="attr-nav">
                    <ul>
                        ...
                        <li>
                            @if (Auth::check())
                                <a
                                    href="{{ route('profile', ['lang' => session('language_code', app()->getLocale())]) }}"><i
                        ...
                        @if (Auth::user())
                            <li>
                                <a
                                    href="{{ route('wishlist.index', ['lang' => session('language_code', app()->getLocale())]) }}">
                            ...
                            <li class="side-menu">
                                <a
                                    href="{{ route('cart', ['lang' => session('language_code', app()->getLocale())]) }}">
                        ...
                        @if (quote() && quote()->quoteItems && quote()->quoteItems->count() > 0)
                            @foreach (quote()->quoteItems as $item)
                                <li>
                                    <a href="{{ route('product', ['lang' => $langCode, 'url_key' => $item->product->url_key]) }}"
                                        class="photo"><img
                                            src="{{ $item->product->getFirstMediaUrl('image') }}?v={{ time() }}"
                                            loading="lazy" class="cart-thumb" alt="" /></a>
                                    <h6> <a
                                            href="{{ route('product.detail', ['lang' => $langCode, 'url_key' => $item->product->url_key]) }}">
                                            {{ $item->product->name }}</h6>
                                    <p>{{ $item->qty }}x - <span
                                            class="price">{{ $symbol }}{{ number_format($item->price * $rate, 2) }}</span>
                                    </p>
                            ...
                            <li class="total">
                                <a href="{{ route('cart', ['lang' => $langCode]) }}"
                                    class="btn btn-default hvr-hover btn-cart">{{ __('buttons.view_cart') }}</a>
                                @php
                                    $cartTotal = quote()->quoteItems->sum(function ($item) use ($rate) {
                                        return $item->price * $item->qty * $rate;
                                    });
                                @endphp

                                <span class="float-right">
                                    <strong>Total</strong>:{{ $symbol }}{{ number_format($cartTotal, 2) }}
                                </span>
            ...
            <style>
                ul.cart-list>li {
                    position: relative;
                    border-bottom: solid 23px #efefef;
                    padding: 7px 5px 30px 15px !important;
                }
            </style>
        </nav>
    </header>
